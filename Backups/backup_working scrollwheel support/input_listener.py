...
def periodic_reload(interval=1.0):
    while True:
        load_macros()
        time.sleep(interval)

def execute_macro(macro, button_vk=None):
    if button_vk is not None and active_flags.get(button_vk):
        print(f"[SKIP] Macro already running for VK {button_vk}")
        return
    if button_vk is not None:
        active_flags[button_vk] = True
    ...
    def run():
        try:
            ...
            elif mode == "loop" and button_vk:
                loop_flags[button_vk] = True
                while loop_flags.get(button_vk, False):
                    run_macro(actions, original_vk=button_vk)
                    time.sleep(0.015)
            else:
            ...
            if button_vk is not None:
                active_flags[button_vk] = False
                if mode == "loop":
                    loop_flags[button_vk] = False
        try:
            if mode == "hold" and button_vk:
                while hold_flags.get(button_vk, False):
                    run_macro(actions, original_vk=button_vk)
...
def check_and_execute(macro, btn_code, pressed):
    ...
    elif mode == "loop" and pressed:
        if not loop_flags.get(btn_code, False):
            execute_macro(macro, btn_code)
        else:
            loop_flags[btn_code] = False
...
def on_mouse_click(x, y, button, pressed):
    ...
    alias = VK_MAP.get("aliases", {}).get(btn_name, btn_name)
    btn_vk = VK_MAP.get(alias)

    print(f"[DEBUG] Mouse event: {btn_name}, VK: {btn_vk}, Pressed: {pressed}")
    if btn_vk is not None:
        if pressed:
            pressed_keys.add(btn_vk)
        else:
            pressed_keys.discard(btn_vk)
        for macro in macros:
...
def on_scroll(x, y, dx, dy):
    ...
    if dy > 0:
        btn_vk = VK_MAP.get("scroll_up")
    elif dy < 0:
        btn_vk = VK_MAP.get("scroll_down")
    else:
        return
    print(f"[DEBUG] Scroll event: VK={btn_vk}, dy={dy}")

    if btn_vk is not None:
        pressed_keys.add(btn_vk)
        for macro in macros:
            check_and_execute(macro, btn_vk, True)
        pressed_keys.remove(btn_vk)
