...
class Action:
    def __init__(self, action, key=None, ms=None):
        self.action = action
        self.key = key
        self.ms = ms
...
try:
    fixed_delay = int(input("Enter fixed delay between actions in ms (default: 15): ").strip() or 15)
except ValueError:
    fixed_delay = 15
...
# === СОХРАНЕНИЕ — МИНИМАЛИСТИЧНЫЙ СТИЛЬ ===
